...
function RankingView({ navigation }) {
  ...
  return (
    <View className="gap-2.5 p-5">
      ...
      <Pressable onPress={() => navigation.navigate('FullRanking')} className="items-center rounded-item border border-line bg-surface py-3">
...
function FeedView({ toast, cheer, navigation }) {
  ...
  return (
    <SafeAreaView edges={['top']} className="flex-1 bg-bg">
      ...
      <View className="flex-row items-center justify-between px-[30px] pt-4">
        ...
        {tab !== 'info' ? (
          <CoachMascotButton
            onPress={() => navigation.navigate('AiCoach', { from: tab === 'feed' ? 'feed' : 'ranking' })}
            circle={54}
            size={44}
          />
        ) : (
      ...
      <ScrollView className="flex-1" contentContainerClassName="pb-6">
        {tab === 'feed' && <FeedView toast={toast} cheer={cheer} navigation={navigation} />}
        {tab === 'ranking' && <RankingView navigation={navigation} />}
